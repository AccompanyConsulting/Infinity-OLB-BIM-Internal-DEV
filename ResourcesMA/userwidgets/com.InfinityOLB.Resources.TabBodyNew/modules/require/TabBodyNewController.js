	...
	return {
          ...
          setSegmentWidget: function(wid) {
            ...
            if (curr_form === "frmBBApprovalsDashboard") {
                ...
                var featureTransactionId = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].transactionId));
        ...
        } else if (['SIGNATORY_GROUP_CREATE_EDIT', 'SIGNATORY_GROUP_DELETE'].includes(featureActionId)) {
          if (curr_form === "frmBBRequestsDashboard") {
            var flxPendingRequestSignatoryGruop = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxPendingRequestSignatoryGruop));
            if (flxPendingRequestSignatoryGruop.isVisible === true) {
              flxPendingRequestSignatoryGruop.isVisible = false;
            } else {
              flxPendingRequestSignatoryGruop.isVisible = true;
            }
            this.updateKeyAt("flxPendingRequestSignatoryGruop", flxPendingRequestSignatoryGruop, row, section);
          } else {
            var flxHistoryApprovals = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxHistoryApprovals));
            if (flxHistoryApprovals.isVisible === true) {
              flxHistoryApprovals.isVisible = false;
            } else {
              flxHistoryApprovals.isVisible = true;
            }
            this.updateKeyAt("flxHistoryApprovals", flxHistoryApprovals, row, section);
          }
        } else if (featureActionId == 'APPROVAL_MATRIX_MANAGE' && !featureTransactionId.includes("TOGGLE_APPROVAL")) {
          var flxPendingManageApprovalMatrix = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxPendingManageApprovalMatrix));
          if (flxPendingManageApprovalMatrix.isVisible === true) {
            flxPendingManageApprovalMatrix.isVisible = false;
          } else {
            flxPendingManageApprovalMatrix.isVisible = true;
          }
          this.updateKeyAt("flxPendingManageApprovalMatrix", flxPendingManageApprovalMatrix, row, section);
          ...
          var flxPendingManageApprovalToggle  = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxPendingManageApprovalToggle ));
          if (flxPendingManageApprovalToggle .isVisible === true) {
            flxPendingManageApprovalToggle .isVisible = false;
          } else {
            flxPendingManageApprovalToggle .isVisible = true;
          }
          this.updateKeyAt("flxPendingManageApprovalToggle", flxPendingManageApprovalToggle , row, section);
          var sknLineobject = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxDetilsHighlighterMain));
                      sknLineobject.height = "190dp";
                    if (sknLineobject.isVisible === true) {
                        sknLineobject.isVisible = false;
                    ...
                    if ((!(i === section && j === row)) && segData[i][1][j].imgDropDown) {
                        ...
                        if (curr_form === "frmBBApprovalsDashboard") {
                            ...
                            this.updateKeyAt("flxDetilsHighlighterMain", sknObjectOldLine, j, i);
                             ...
                             segData[i][1][j].flxHistoryApprovals.isVisible = false;
                             segData[i][1][j].flxBeneficiaryType.isVisible = false;
                             segData[i][1][j].flxPendingRequestSignatoryGruop.isVisible = false;
                             segData[i][1][j].flxPendingManageApprovalMatrix.isVisible = false;
                             segData[i][1][j].flxPendingManageApprovalToggle.isVisible = false;
         ...
         showOrHideDetailsHeader: function(context) {
            ...
            if (curr_form === "frmBBApprovalsDashboard" || curr_form === "frmBBRequestsDashboard") {
                ...
                var valueTransactionIdtxt = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].transactionId));
                ...
                } else if (['SIGNATORY_GROUP_CREATE_EDIT', 'SIGNATORY_GROUP_DELETE'].includes(valuetxt)) {
                  if (curr_form === "frmBBRequestsDashboard") {
                    var flxPendingRequestSignatoryGruop = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxPendingRequestSignatoryGruop));
                    if (flxPendingRequestSignatoryGruop.isVisible === true) {
                      flxPendingRequestSignatoryGruop.isVisible = false;
                    ...
                    var flxPendingApprovals = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxPendingApprovals));
                    if (flxPendingApprovals.isVisible === true) {
                      flxPendingApprovals.isVisible = false;
                    } else {
                      flxPendingApprovals.isVisible = true;
                    }
                    this.updateKeyAt("flxPendingApprovals", flxPendingApprovals, row, section);
                  }
                } else if (valuetxt == 'APPROVAL_MATRIX_MANAGE'&& !valueTransactionIdtxt.includes("TOGGLE_APPROVAL")) {
                  var flxPendingManageApprovalMatrix = JSON.parse(JSON.stringify(this._segWidget.data[section][1][row].flxPendingManageApprovalMatrix));
                  if (flxPendingManageApprovalMatrix.isVisible === true) {
                    flxPendingManageApprovalMatrix.isVisible = false;
                  } else {
            ...
            for (var i = 0; i < segData.length; i++) {
                for (var j = 0; j < segData[i][1].length; j++) {
                    if (j === row) {} else {
                        ...
                        segData[i][1][j].flxDetilsHighlighterMain.isVisible = false;
                        segData[i][1][j].flxTransactionTypes.isVisible = false;
                        segData[i][1][j].flxPendingApprovals.isVisible = false;
                        segData[i][1][j].flxBeneficiaryType.isVisible = false;
                        segData[i][1][j].flxPendingRequestSignatoryGruop.isVisible = false;
                        segData[i][1][j].flxPendingManageApprovalMatrix.isVisible = false;
                        segData[i][1][j].flxPendingManageApprovalToggle.isVisible = false;
