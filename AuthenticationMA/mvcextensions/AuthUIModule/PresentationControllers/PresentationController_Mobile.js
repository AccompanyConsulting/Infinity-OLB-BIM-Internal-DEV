define(["CommonUtilities"], function(CommonUtilities) {
  function Auth_PresentationController() {
    ...
    this.isTnCRequirePostConsentPermission = true;
    this.userList = [];
    ...
    this.permissionData = {};
    this.isNewLogintobeShown = false;
    this.isResumeApplicationShown = true;
    this.isloginFormShown = false;
    this.isWealthUser = "false";
    this.isMultiCustomer = "false";
    this.isTeePeeNavigationRequired = true;
    this.deeplinkMenu = "";
	this.consentId = "";
  }
...
//    }
  ...
  Auth_PresentationController.prototype.postLoginServicesSuccess = function(){
    if(scope_AuthPresenter.isTnCRequirePostConsentPermission == false){
        return;
    }
  ...
  Auth_PresentationController.prototype.navigationAfterLogin = function() {
    ...
    if(!kony.sdk.isNullOrUndefined(custominfoCD) && custominfoCD.reDesignFlow === "true" && !wealthInvestmentDetail){
        if(applicationManager.gblDeepLink === "true"){
            var dashboard = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule({
                "moduleName": "AccountsUIModule",
                "appName": "HomepageMA"
            });
            dashboard.presentationController.getPSD2Accounts();  //for App2App
        }
		else{
            this.navigateToOverviewSuccess();
        }
  } else if(wealthInvestmentDetail || wealthPortfolioDetail){
        this.isWealthUser = "true";
        custominfoCD.isWealthUser = this.isWealthUser;
        let backendID = applicationManager.getUserPreferencesManager().getBackendIdentifier();
        let coreCustomers = applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers;
        if(!kony.sdk.isNullOrUndefined(backendID) && !kony.sdk.isNullOrUndefined(coreCustomers)){       
        let customerLength = coreCustomers.length;
        let coreCustomerID = coreCustomers[0].coreCustomerID;
	//	let primaryUser = applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].isPrimary;
        if(customerLength > 1 || ((backendID !== "" && backendID !== undefined && backendID !== null )  && (coreCustomerID !== backendID)) || this.isMultiCustomer === "true") {
        this.isMultiCustomer = "true";
        custominfoCD.isMultiCustomer = this.isMultiCustomer;
        const isPortfolioMAPresent = configManager.isMicroAppPresent('PortfolioManagementMA');
        if(isPortfolioMAPresent){
         var portfolio = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule({
      ...
      var param = {
       ...
       portfolio.getCustomers(param);
        }
        }
        else{
          if(custominfoCD === undefined){
            custominfoCD = {}
            }
            this.isMultiCustomer = "false";
            custominfoCD.isMultiCustomer = this.isMultiCustomer;
...
//       });
  ...
  Auth_PresentationController.prototype.navigateToOverviewSuccess = function(res) {
   ...
   // var configurationArray = res;
    ...
    var custominfo = navManager.getCustomInfo("frmCustomerDashboard");
    var accountData = (this.isWealthUser === "true") ? res.Customers : custominfo.accountData;
    custominfo.reDesignFlowWealth = (this.isWealthUser === "true") ? 
    ((this.isMultiCustomer === "true") ? "true" : "false") : custominfo.reDesignFlow;
    custominfo.accountData = accountData;
  ...
  Auth_PresentationController.prototype.presentationUserFeaturesAndPermissionsSuccess = function (resSuccess) {
      ...
      let features = JSON.parse(applicationManager.getConfigurationManager().features);
	  if((applicationManager.gblDeepLink === "true") && ((features.indexOf("OPEN_BANKING_ACCESS") === -1) || (features.indexOf("OPEN_BANKING_ACCOUNT_CONSENT") === -1))){
		applicationManager.getPresentationUtility().dismissLoadingScreen();
		accMode.presentationController.consentStatus = "Denied";
        scope_AuthPresenter.isTnCRequirePostConsentPermission = false;
        navManager.setCustomInfo("frmConsentWarning", "You do not have sufficient permissions.");
        var frmName = {"appName": "HomepageMA","friendlyName": "AccountsUIModule/frmAuthConsentWarning"};         
		navManager.navigateTo(frmName); 		
		return;
	  }
  ...
  Auth_PresentationController.prototype.presentationUserSuccess = function (resUserSucess) {
    ...
    if (storageManager.getStoredItem("registerForPushNotifications")) {
      ...
      //As per suggestion from Quantum Team, commenting the below line and 
      //placed it in deregistrationsuccesscallback TSR-778715
      //regManager.registerForPushNotifications();
    }
  };
 ...
 Auth_PresentationController.prototype.saveUserCredentialsOfExternalBank = function(username, password, sessionToken, mainUser, bankId, successCallback, errorCallback) {
  ...
  Auth_PresentationController.prototype.fetchActionsList = function() {
     ...
     */
    Auth_PresentationController.prototype.validateAuthToken = function(authToken) {
        var userPrefManager = applicationManager.getUserPreferencesManager();
        var params = {
            "authToken" : authToken        
        };
        userPrefManager.validateAuthToken(params,this.validateAuthTokenSuccess.bind(this), this.validateAuthTokenError);
    };
    /**
     * validateAuthToken success call back
     */
    Auth_PresentationController.prototype.validateAuthTokenSuccess = function(data) {
        console.log("validateAuthTokenSuccess");
        this.deeplinkMenu = "accountConsent";
		this.consentId = data[0].consentId;
		applicationManager.consentTypeName = data[0].accountConsentType;
        // if("AISP" === data[0].consentType){
        //     this.deeplinkMenu = "frmConfirmAccountConsent";
        // }else if("PISP" === data[0].consentType){
        //     this.deeplinkMenu = "frmPaymentConsent";
        // }
    };
    /**
     * validateAuthToken error call back
     */
    Auth_PresentationController.prototype.validateAuthTokenError = function(error) {
        console.log("validateAuthTokenError");
    };
