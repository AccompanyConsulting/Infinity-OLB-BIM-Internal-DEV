    ...
    AuthPresentationController.prototype.initializePermissions = function () {
      this.getUserandSecurityAttributes(function(params) {
        if(params !==undefined && params !== null){
          ...
          try {
            kony.mvc.MDAApplication.getSharedInstance().appContext.isCSR_Assist_Mode = userAttributes.isCSRAssistMode ? userAttributes.isCSRAssistMode.toLowerCase() === 'true' : kony.mvc.MDAApplication.getSharedInstance().appContext.isCSR_Assist_Mode;
          } catch (e) {
          }
 ...
 */
    ...
    */
    ...
    AuthPresentationController.prototype.navigateToNewUserOnBoarding = function () {
        ...
        var embeddedOrigination = configurationManager.getEmbeddedOrigination();
        if (reDirectionURL) {
            // location.assign(reDirectionURL);
            if(embeddedOrigination.toLowerCase() === 'iframe'){
                applicationManager.getNavigationManager().navigateTo("frmEmbedOrigination");
            } else {
    ...
    */
    ...
    AuthPresentationController.prototype.getPreLoginCampaignsOnBreakpointChange = function () {
        ...
        if(Object.keys(clientProperties).length === 0) {
            ...
            configurationManager.setEmbeddedOriginationType(response["EMBED_ORIGINATION"]);
			...
			configurationManager.setEnableHeavyUserDasboard(response["ENABLE_HEAVY_USER_DASHBOARD"]);
