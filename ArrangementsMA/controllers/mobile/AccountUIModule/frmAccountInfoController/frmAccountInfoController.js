...
define(['CampaignUtility', 'CommonUtilities'], function(CampaignUtility, CommonUtilities){
  ...
  onNavigate: function(obj) {
    ...
    var ending = ".";
              ...
              var accountDetail = navMan.getCustomInfo("frmAccountDetailsInfo");
            //   var list = ["MUDARABA.DAILY.MIN.ACCOUNT"];
                ...
                for (var cont in contextObj) {
                   ...
                   if (key === "accountType") {
                      if(!kony.sdk.isNullOrUndefined(accountDetail["productType"]) && accountDetail["productType"].includes("ISLAMIC") == true){
                        contextObj[cont + "New"] = contextObj[cont] + "Islamic";
                      }else{
                        contextObj[cont + "New"] = contextObj[cont];
                      }
                    }
                    if (key === "interestRate" || key === "dividendRate") {
                        contextObj[index[0] + ".dividendRate"] = contextObj[index[0] + ".dividendRate"] ? contextObj[index[0] + ".dividendRate"] : contextObj[index[0] + ".interestRate"];
                    }
  ...
  preshowAccInfo: function() {
    ...
    this.view.ProfitRatePopup.setVisibility(false);
    this.view.flxContainer.onScrolling = function() {
             scope.iPhoneHeaderHandler();
              ...
              scope.view.information.onLinkClick = function(msg) {
                  scope.showProfitRatePopup(msg);
              };
              scope.view.ProfitRatePopup.imgClose.onTouchStart = function(){
                scope.view.ProfitRatePopup.setVisibility(false);
              }
...
//         authModule.presentationController.launchExternalBankLogin(selectedItem);
  ...
  showProfitRatePopup: function(msg){
    this.view.ProfitRatePopup.lblHeading.text = kony.i18n.getLocalizedString("i18n.accountDetail.ProfitRate");
    msg = msg.split("\n");
    var data = [];
    for(key in msg){
        var temp = { "imgDot": "pageoffdot_9.png",
                    "lblDetails" : msg[key]}
        data.push(temp);
    }
    this.view.ProfitRatePopup.segInterestDetails.setData(data);
    this.view.ProfitRatePopup.setVisibility(true);
  }
  };
});
