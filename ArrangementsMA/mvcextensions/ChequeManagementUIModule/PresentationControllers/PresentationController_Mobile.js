            ...
            for (var i = 0; i < response.length; i++) {
             ...
             var chequeBooks = "";
                if (response[i].numberOfChequeBooks === null || response[i].numberOfChequeBooks === undefined ||isNaN(response[i].numberOfChequeBooks)) {
                  ...
                  chequeBooks = response[i].numberOfChequeBooks + " " + kony.i18n.getLocalizedString("kony.mb.CM.book(s)")
    ...
    if(Array.isArray(response)){
      for(var i=0;i<response.length;i++){
        ...
        data.checkDateOfIssue=checkDateOfIssue;
          ...
          var trandateofissue = formatUtil.getDateObjectfromString(date, "YYYY-MM-DD");
          var checkDateOfIssue = formatUtil.getFormatedDateString(trandateofissue, formatUtil.getApplicationDateFormat());
          data.checkDateOfIssue=checkDateOfIssue;
        }
    ...
    if(Array.isArray(response)){
      for (var i = 0; i < response.length; i++) {
        ...
        data.booksCount = response[i].checkNumber;
         data.lblDate =
              {"text":formatUtil.formatAmountandAppendCurrencySymbol(response[i].amount, scope_ChequePresentationController.currencyCode),
                "skin":"sknlbl42424240px"}
