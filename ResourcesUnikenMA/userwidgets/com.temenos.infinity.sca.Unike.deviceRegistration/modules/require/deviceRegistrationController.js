define(['./deviceRegistrationDAO', 'CommonUtilities', 'FormControllerUtility'], function (deviceRegistrationDAO, CommonUtilities, FormControllerUtility) {
  ...
  return {
    constructor: function (baseConfig, layoutConfig, pspConfig) {
      ...
      // Other vars
      this.logoutReq = false;
      this.removeDeviceReq = false;
    },
    ...
    postShowDeviceRegistration: function () {
      // this.fetchDevices();
    },
    ...
    updateComponentUI: function (context) {
      ...
      if (context) {
        ...
        if (context.SCADevice) {
          this.setDevices(context.SCADevice);
        }
      }
    },
    ...
    */
    ...
    setDevices: function (devices) {
      ...
      segData = devices.map(function (data) {
        ...
        var timeStamp = Number(data.createdTs);
        const RegisteredDate = new Date(timeStamp);
        //let formattedDate = CommonUtilities.getDateAndTime(RegisteredDate).substring(0, 10);
        let formattedDate = RegisteredDate.toLocaleDateString("en-GB");
        return {
    ...
    getServiceCallParam: function (operationName) {
      ...
      if (operationName === this._operationName1) {
        param = {
          "deviceUUID": this.currentSelectedRowItems.deviceUUID,
          "serviceName": this.currentSelectedRowItems.deviceStatus === "ACTIVE" ? "SUSPEND_DEVICE" : "UNSUSPEND_DEVICE",
          "status": this.currentSelectedRowItems.deviceStatus === "ACTIVE" ? "BLOCK" : "UNBLOCK",
          "deviceName": this.currentSelectedRowItems.deviceName
        };
      }
    ...
    removeConfirmationPopup: function (isRequestFromRemoveDevice) {
      ...
      this.view.forceLayout();
      if (isRequestFromRemoveDevice && this.logoutReq) {
        this.logoutReq = true;
      } else if (this.logoutReq) {
        this.logoutReq = false;
      }
    },
    ...
    addConfirmationPopup: function (popupTxt) {
      ...
      let deviceRegistrationPopup = new com.temenos.infinity.sca.Uniken.deviceRegistrationPopup({
        "height": "100%",
        ...
        "overrides": {
          ...
          "flxClosePopup": {
            "onClick": function () {
              self.removeConfirmationPopup(false);
            }
          },
          "btnNo": {
          ...
          "btnYes": {
            ...
            "onClick": function () {
              self.removeDeviceReq = true;
              self.removeConfirmationPopup(true);
    ...
    addTopSuccessErrMsg: function (msg, isSuccess) {
      ...
      if (self.removeDeviceReq && isSuccess) {
        self.removeDeviceDataFromSeg(self.view.segRegisteredDevices.data, self.currentSelectedRowItems.deviceUUID);
      }
      self.removeDeviceReq = false;
      var topMessage = new com.temenos.infinity.sca.uniken.topMessage({
        "appName": "ResourcesUnikenMA",
    ...
    addCIBAStatusLoadingScreen: function () {
      ...
      currform.add(Popup);
      if (currform.flxHeader.info.frame && currform.flxFooter.info.frame && currform.flxContainer.info.frame)
        currform.UnikenPopup.height = currform.flxHeader.info.frame.height + currform.flxFooter.info.frame.height + currform.flxContainer.info.frame.height + "dp";
      currform.scrollToWidget(currform.flxHeader);
    ...
    removeCIBAStatusLoadingScreen: function () {
      var currform = kony.application.getCurrentForm();
      if (currform.UnikenPopup) {
        currform.UnikenPopup.setVisibility(false);
        currform.remove(currform.UnikenPopup);
      }
      this.view.forceLayout();
    },
    ...
    startFetchCIBAStatus: function (data) {
      ...
      kony.timer.schedule("cibatimer", function () {
        scopeObj.fetchCIBAStatus(data.auth_req_id);
      }, 20, true);
    },

    stopFetchCIBAStatus: function (data) {
      let currentTime = new Date().getTime(), msg;
      if (data && data.action_response === "Accept") {
        kony.timer.cancel("cibatimer");
        this.removeCIBAStatusLoadingScreen();
        this.serviceCallBasedOnOperation(this.currentOperationName, { "auth_req_id": data.auth_req_id });
        msg = this.getSuccessErrMsgText(data.action_response, this.currentOperationName, this.currentSelectedRowItems);
        this.addTopSuccessErrMsg(msg, true);
      }
      // cancel after 2 minutes
      else if (currentTime - this.startTime > 120000 || data.action_response === "Deny") {
        this.removeCIBAStatusLoadingScreen();
        kony.timer.cancel("cibatimer");
        msg = this.getSuccessErrMsgText(data.action_response, this.currentOperationName, this.currentSelectedRowItems);
        this.addTopSuccessErrMsg(msg, false);
    ...
    fetchDevices: function () {
      this.view.segRegisteredDevices.setData([]);
      if (this.logoutReq) {
        // Auto logout the user if no device is available
        this.view.segRegisteredDevices.setVisibility(false);
        this.view.flxlNoRecords.setVisibility(true);
        applicationManager.getAuthManager().logout(this.logoutSuccessCallback.bind(this), this.logoutErrorCallback.bind(this));
      } else {
    ...
    serviceCallBasedOnOperation: function (operationName, param) {
      if (operationName === this._operationName1) {
        param.deviceUUID = this.currentSelectedRowItems.deviceUUID,
          param.status = this.currentSelectedRowItems.deviceStatus === "ACTIVE" ? "BLOCK" : "UNBLOCK",
    ...
    removeDeviceDataFromSeg: function (segData, selectedDeviceUUID) {
      var scope = this;
      try {
        let tempSegData = segData;
        segData.map((device, index) => {
          if (device.deviceUUID === selectedDeviceUUID) {
            tempSegData.splice(index, 1); // remove 1 element at index
            scope.totalActiveDevices--;
            scope.view.segRegisteredDevices.setData(tempSegData);
          }
        });
      } catch (err) {
          var errorObj = {
              "method": "removeDeviceDataFromSeg",
              "error": err
          };
          kony.print(errorObj);
      }
