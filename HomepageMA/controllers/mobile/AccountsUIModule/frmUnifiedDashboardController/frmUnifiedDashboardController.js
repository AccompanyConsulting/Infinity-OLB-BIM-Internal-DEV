define(['CampaignUtility', 'CommonUtilities'], function(CampaignUtility, CommonUtilities) {
  ...
  return {
    ...
    preshow: function() {
      ...
      if (investmentPermission) {
        ...
        if(isAppPresent===true){
         ...
         /* if(applicationManager.getConfigurationManager().wealthBankDate.toString().toLowerCase() === "true"){
            this.getBankDate();
            }
      ...
      var isAppPresent=configManager.isMicroAppPresent("PortfolioManagementMA");
      if(isAppPresent===true){
        if (applicationManager.getConfigurationManager().wealthBankDate === undefined || applicationManager.getConfigurationManager().wealthBankDate === null || applicationManager.getConfigurationManager().wealthBankDate === '') {
                applicationManager.getConfigurationManager().wealthBankDate = true;
            }
          if(applicationManager.getConfigurationManager().wealthBankDate.toString().toLowerCase() === "true"){
            this.getBankDate();
            }
	  ...
	  if (applicationManager.getPresentationFormUtility().getDeviceName() !==	"iPhone") {
            ...
            if(!kony.sdk.isNullOrUndefined(custominfoCD) && (custominfoCD.reDesignFlow === "true" || custominfoCD.reDesignFlowWealth === "true")){
              ...
              this.view.setLeftBarButtonItems({
               ...
               kony.application.getCurrentForm().titleBarAttributes.leftBarButtonItems[0].action = this.flxBackOnClick; 
            }else{
                leftBarButtonItem = new kony.ui.BarButtonItem({
                type: constants.BAR_BUTTON_IMAGE,
                style: constants.BAR_ITEM_STYLE_PLAIN,
    ...
    resetLanguageChange: function(){
      let custominfoCD = applicationManager.getNavigationManager().getCustomInfo("frmCustomerDashboard");
	  if(!kony.sdk.isNullOrUndefined(custominfoCD) && (custominfoCD.reDesignFlow === "true" || custominfoCD.reDesignFlowWealth === "true")){
        if(custominfoCD.isWealthUser === "true"){
            this.view.customHeader.lblLocateUs.text = custominfoCD.selectedRow.customerName.text+" ("+ custominfoCD.selectedRow.customerId.text +")";
         this.view.title = custominfoCD.selectedRow.customerName.text+" ("+ custominfoCD.selectedRow.customerId.text +")";
        }else{
      	this.view.customHeader.lblLocateUs.text = custominfoCD.MembershipName;
        this.view.title = custominfoCD.MembershipName;
        }
       //  this.view.customHeader.lblLocateUs.text = applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerName+" ("+applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerID+")";
       // this.view.title = applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerName+ " ("+applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerID+")";
      }else{
        /*if(custominfoCD.isWealthUser === "true"){
            this.view.customHeader.lblLocateUs.text = applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerName+" ("+applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerID+")";
            this.view.title = applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerName+ " ("+applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerID+")";
        ...
        accounts[key].forEach(function(account, i) {
          ...
          account.lblAccountBal = {
            ...
            "skin": (account.availableBalance < 0 && (configurationManager.constants.CHECKING === account.accountType || configurationManager.constants.SAVINGS === account.accountType))
                      ? "sknLblREDSSPReg22px" : "ICSknLbl727272SSPReg22px"
          };
    ...
    mapAccountsAndBanks: function() {
      try {
        ...
        var custDashboard = applicationManager.getNavigationManager().getCustomInfo("frmCustomerDashboard");
        if (custDashboard.isMultiCustomer === "true") {
            try {
                var customerId = custDashboard.selectedRow.customerId.text  ;
...
//           this.view.flxPageIndicators.top = "210dp";
      ...
      try {
        if (this.showPwdWarningFlag && !(!kony.sdk.isNullOrUndefined(custominfoCD) && (custominfoCD.reDesignFlow === "true" || custominfoCD.reDesignFlowWealth  === "true"))) {
          this.showPwdWarning();
        }
...
//           scope.view.flxPageIndicators.top = Math.round(vScroll * -0.3) + 230 + "dp";
        ...
        if (applicationManager.getPresentationFormUtility().getDeviceName() === "iPhone") {
          scope.view.flxDummyHorizontalScroll.animate(kony.ui.createAnimation({
            ...
            animationEnd: function () {
              scope.expandAnimationEnd(scope);
          ...
          scope.view.flxDashboard.animate(kony.ui.createAnimation({
            ...
            animationEnd: function () {
              scope.view.flxMenu.animate(kony.ui.createAnimation({
                ...
                animationEnd: function () {
                  scope.collapseAnimationEnd(scope);
                  scope.view.flxDummyHorizontalScroll.animate(kony.ui.createAnimation({
                    0: {
    ...
    expandAnimationEnd: function (scope) {
      var newHeight = scope.view.flxBGDummy.frame.height;
      if (applicationManager.getPresentationFormUtility().getDeviceName() === "android") {
        newHeight = newHeight - 56;
      }
    ...
    collapseAnimationEnd: function (scope) {
    ...
    showSearchAccounts: function() {
      ...
      if (this.view.flxSearchContainer.isVisible) {
        this.view.flxInnerChartSizeToggle.isVisible = true;
        this.view.flxSearchContainer.isVisible = false;
        this.view.flxSelectedAccounts.isVisible = true;
        this.showorHideScrollingMenu(true);
        var configManager = applicationManager.getConfigurationManager();      
        ...
        var scope = this;
        this.view.flxInnerChartSizeToggle.isVisible = false;
    ...
    accountSegmentOnClick: function() {
      ...
      var contextData = {
        ...
        "productId": (rowid["productId"]) ? rowid["productId"] : "",
        "productType": (rowid["productType"]) ? rowid["productType"] : "",
      }
      ...
      navManager.setCustomInfo("frmAccountDetailsInfo", contextData);
    ...
    goToRecentActivity: function() {
      try{
      var inputParams = {
        "coreCustomerId": this.customerId?this.customerId:"",
        "bankDate": scope_WealthPresentationController.bankdate?scope_WealthPresentationController.bankdate:""
      }
    ...
    goToPortfolioDetails: function(portfolioId, refCurrencyId) {
     ...
     var params = {
         ...
         wealthModule.getPortfolioPermission(params);
    ...
    getBankDate : function(){
        try{
            var wealthModule = kony.mvc.MDAApplication.getSharedInstance().moduleManager.getModule({ "moduleName": "WealthPortfolioUIModule", "appName": "PortfolioManagementMA" }).presentationController;
            wealthModule.getBankDate();
      }catch(err) {
        this.setError(err, "getBankDate");
      }
    ...
    getAssetsList: function() {
      ...
      var custominfoCD = applicationManager.getNavigationManager().getCustomInfo("frmCustomerDashboard");
      var params = {
        "coreCustomerId": (!kony.sdk.isNullOrUndefined(custominfoCD)  && custominfoCD.isMultiCustomer === "false")?"":this.customerId
      };
    ...
    checkPermissions: function() {
      ...
      if (marketIndexPermission === false || scope_WealthPresentationController.marketIndexService === false) {
        ...
        if(isAppPresent===true){
          ...
          this.view.noAccounts.isVisible = false;
    ...
    flxBackOnClick: function() {
        var custominfoCD = applicationManager.getNavigationManager().getCustomInfo("frmCustomerDashboard");
        var navManager = applicationManager.getNavigationManager();
        if(!kony.sdk.isNullOrUndefined(custominfoCD) &&!kony.sdk.isNullOrUndefined(custominfoCD.isWealthUser) && custominfoCD.isWealthUser === "true"){
            custominfoCD.reDesignFlowWealth = "false";
            custominfoCD.isBackClicked = "true";
            applicationManager.getNavigationManager().setCustomInfo("frmCustomerDashboard", custominfoCD);
            var authMA = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule({
             "moduleName": "AuthUIModule",
             "appName": "AuthenticationMA"
           }).presentationController;
	    authMA.navigationAfterLogin();
        }else{
