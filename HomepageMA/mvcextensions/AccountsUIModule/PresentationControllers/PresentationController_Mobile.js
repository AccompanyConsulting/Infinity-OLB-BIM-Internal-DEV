define(["CommonsMA/AsyncManager/BusinessControllers/BusinessController", "dataFormatUtility", "CommonUtilities", "OLBConstants"], function(AsyncManager, dataFormater, CommonUtilities, OLBConstants) {
  ...
  */
  ...
  Account_PresentationController.prototype.showDashboard = function() {
    ...
    if((!(!kony.sdk.isNullOrUndefined(custominfoCD) && (custominfoCD.reDesignFlow === "true"))) && (applicationManager.gblDeepLink !== "true"))
    applicationManager.getPresentationUtility().showLoadingScreen();
    var accountManager = applicationManager.getAccountManager();
    var configManager = applicationManager.getConfigurationManager();
    ...
    if(investmentPermission===true && isAppPresent===true){
      ...
      if (userMan.backendIdentifier && userMan.backendIdentifier  !== "") {
        ...
        wealthModule.getPortfolioList({
          "coreCustomerId": (!kony.sdk.isNullOrUndefined(custominfoCD)  && custominfoCD.isMultiCustomer === "false")?"":customerId
        });
...
//	if(!(!kony.sdk.isNullOrUndefined(custominfoCD) && (custominfoCD.reDesignFlow === "true")))   
  ...
  Account_PresentationController.prototype.getPSD2Accounts = function(accts) {
    var accountManager = applicationManager.getAccountManager();
    var authMode = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule("AuthUIModule");
    var params = {
      "consentId": authMode.presentationController.consentId,
      "customerId": applicationManager.getUserPreferencesManager().getUserObj().CoreCustomers[0].coreCustomerID//"190627"
    };
    accountManager.getPSD2Accounts(params, scope_Acc_Pres.getPSD2AccountsSucc, scope_Acc_Pres.getPSD2AccountsErr);
  };

  Account_PresentationController.prototype.getPSD2AccountsSucc = function(res) {
	var navManager = applicationManager.getNavigationManager();
    if (!kony.sdk.isNullOrUndefined(res) && res !== "") {
		navManager.setCustomInfo("frmAuthConsent", res);
		var frmName = {"appName": "HomepageMA","friendlyName": "AccountsUIModule/frmAuthoriseGlobalConsent"}; 
		navManager.navigateTo(frmName);		
	}else{
        scope_Acc_Pres.consentStatus = "Denied";
        navManager.setCustomInfo("frmConsentWarning", "You have no accounts available or eligible");
  ...
  Account_PresentationController.prototype.getPSD2AccountsErr = function(err) {
    kony.print(err);
    var navManager = applicationManager.getNavigationManager();
    var errMessage = "";    
    if (!kony.sdk.isNullOrUndefined(err.errorMessage) && err.errorMessage !== "") {
        scope_Acc_Pres.consentStatus = "Denied";
        errMessage = err.errorMessage;
    }else{
        scope_Acc_Pres.consentStatus = "Server Error";
        errMessage = kony.i18n.getLocalizedString("i18n.tppconsent.serverError");
    }
    navManager.setCustomInfo("frmConsentWarning", err.errorMessage);
    var frmName = {"appName": "HomepageMA","friendlyName": "AccountsUIModule/frmAuthConsentWarning"}; 
	navManager.navigateTo(frmName);
    applicationManager.getPresentationUtility().dismissLoadingScreen();
  };
  
  Account_PresentationController.prototype.approveConsent = function() {    
    applicationManager.getPresentationUtility().showLoadingScreen();
    ...
    accountManager.approveConsent(params, scope_Acc_Pres.approveConsentSucc, scope_Acc_Pres.approveConsentErr);
  };

  Account_PresentationController.prototype.approveConsentSucc = function(res) {
    applicationManager.getPresentationUtility().dismissLoadingScreen();
    scope_Acc_Pres.consentStatus = "Approved";  
    let navManager = applicationManager.getNavigationManager();
    navManager.navigateTo({"appName": "HomepageMA","friendlyName": "AccountsUIModule/frmAuthConsentAcknowledgement"});
  };

  Account_PresentationController.prototype.approveConsentErr = function(err) {
    kony.print(err);
    applicationManager.getPresentationUtility().dismissLoadingScreen();
    let navManager = applicationManager.getNavigationManager();
    scope_Acc_Pres.consentStatus = "Server Error";
    navManager.setCustomInfo("frmConsentWarning", kony.i18n.getLocalizedString("i18n.tppconsent.serverError"));    
    navManager.navigateTo({"appName": "HomepageMA","friendlyName": "AccountsUIModule/frmAuthConsentWarning"});
  //  if(error["isServerUnreachable"])
  //    applicationManager.getPresentationInterruptHandler().showErrorMessage("postLogin", err);
  };
  
  Account_PresentationController.prototype.denyConsent = function() {    
    applicationManager.getPresentationUtility().showLoadingScreen();    
    var authMode = kony.mvc.MDAApplication.getSharedInstance().getModuleManager().getModule({"moduleName" : "AuthUIModule","appName" :"AuthenticationMA"});     
    var accountManager = applicationManager.getAccountManager();
    var params = {
    ...
    accountManager.denyConsent(params, scope_Acc_Pres.denyConsentSucc, scope_Acc_Pres.denyConsentErr);
  };

  Account_PresentationController.prototype.denyConsentSucc = function(res) {
    applicationManager.getPresentationUtility().dismissLoadingScreen(); 
    scope_Acc_Pres.consentStatus = "Denied";
    let navManager = applicationManager.getNavigationManager();
    navManager.setCustomInfo("frmConsentWarning", "You have cancelled the consent request");
    navManager.navigateTo({"appName": "HomepageMA","friendlyName": "AccountsUIModule/frmAuthConsentWarning"});   
  };

  Account_PresentationController.prototype.denyConsentErr = function(err) {
    kony.print(err);
    applicationManager.getPresentationUtility().dismissLoadingScreen();
    let navManager = applicationManager.getNavigationManager();
    scope_Acc_Pres.consentStatus = "Server Error";
  ...
  Account_PresentationController.prototype.denyConsentPreLogin = function() {    
    applicationManager.getPresentationUtility().showLoadingScreen(); 
    ...
    //accountManager.denyConsentPreLogin(params, scope_Acc_Pres.denyConsentPreLoginSucc, scope_Acc_Pres.denyConsentPreLoginErr);
    userPrefManager.deleteConsent(params, scope_Acc_Pres.denyConsentPreLoginSucc, scope_Acc_Pres.denyConsentPreLoginErr);
  };

  Account_PresentationController.prototype.denyConsentPreLoginSucc = function(res) {
    applicationManager.getPresentationUtility().dismissLoadingScreen(); 
    scope_Acc_Pres.consentStatus = "Denied";
    var url = applicationManager.deeplinkSchemaName+"://?status=Denied";
	kony.application.openURL(url);
    if(applicationManager.getPresentationFormUtility().getDeviceName()!="iPhone")
	kony.application.exit(); 
  };

  Account_PresentationController.prototype.denyConsentPreLoginErr = function(err) {
    kony.print(err);
    applicationManager.getPresentationUtility().dismissLoadingScreen();
    var url = applicationManager.deeplinkSchemaName+"://?status=Denied";
	kony.application.openURL(url);
